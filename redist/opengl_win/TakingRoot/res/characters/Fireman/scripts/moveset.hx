...
function doAttack(){
    if(Key.isDown(Inputs.get("Down"))){
        ...
        if(Character.direction == RIGHT) Character.addHitbox("Slash",Character.getCenter().x + 30, Character.getCenter().y, 100, 20, 0, 0.15);
        else Character.addHitbox("Slash",Character.getCenter().x - 30, Character.getCenter().y, 100, 20, 0, 0.15);
    }
}
